...
/* TIER PAYBACK DOUBLES, which is Cookie Clicker's shape. Theirs go 2.5min,
   ...
   The Lurker keeps $1,000; the rest are set against it.
...
function sPicks(def, st){ return Math.min(10, def.picks + st.picks); }
/* Exact, not estimated: both paytables are solved so k numbers returns
   exactly k(k+1)/4 times the bet, so one draw is worth bet x that. */
...
function resolveDraws(def, st, n, budget){
  ...
  for (var i=0;i<live;i++){
    var d = draw10(), h = 0;
    for (var j=0;j<10;j++) if (st.board.indexOf(d[j]) !== -1) h++;
    var m = tab[h] || 0;
    gained += betC * m;
  ...
  var rest = n - live;
  if (rest > 0){
    /* THE REMAINDER SETTLES AT ITS EXACT EXPECTATION, and the quarters are
       carried rather than rounded. A draw is worth bet * k(k+3)/4 cents,
       which for half the pick counts is not a whole number — so the whole
       numerator is accumulated and only whole cents are paid out, with the
       leftover quarters (0 to 3) held on the streamer until they add up.
       Rounding here instead would leak money every single tick. */
    var num = rest * betC * k * (k + 1) + st.centsCarry;
    gained += Math.floor(num / 4);
    st.centsCarry = num % 4;
  }
  st.draws += n;
  return gained;
